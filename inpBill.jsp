...
// Generate Patient Bill ID automactically generate 
...
// List to store patient IDs from database
...
try {
    ...
    // Query to get patient IDs from InPatients table
    String query = "SELECT patient_id FROM InPatients";
    Statement stmt = conn.createStatement();
    ResultSet rs = stmt.executeQuery(query);
    
    // Populate the patientIds list
    while (rs.next()) {
...
<body>
    ...
    <div class="bill-details">
        ...
        <form id="billForm" method="post">
            ...
            <input type="number" id="doctorCharges" name="doctorCharges" value="<%= doctorCharges %>" step="0.01" min="0">

            <label for="medicineCharges">Medicine Charges:</label>
            <input type="number" id="medicineCharges" name="medicineCharges" value="<%= medicineCharges %>" step="0.01" min="0">

            <label for="serviceCharges">Service Charges:</label>
            <input type="number" id="serviceCharges" name="serviceCharges" value="<%= serviceCharges %>" step="0.01" min="0">

            <label for="roomCharges">Room Charges:</label>
            <input type="number" id="roomCharges" name="roomCharges" value="<%= roomCharges %>" step="0.01" min="0">

            <label for="hospitalCharges">Hospital Charges:</label>
            <input type="number" id="hospitalCharges" name="hospitalCharges" value="<%= hospitalCharges %>" step="0.01" min="0">

            <label for="discount">Discount:</label>
            <input type="number" id="discount" name="discount" value="<%= discount %>" step="0.01" min="0">

            <label for="netValue">Net Value:</label>
            <input type="number" id="netValue" name="netValue" value="<%= netValue %>" step="0.01" min="0" readonly>
    ...
    <script>
        // Calculate net value when any charge or discount changes
